...
#ifndef NVMDB_OPTIONS_H_
#define NVMDB_OPTIONS_H_
...
namespace nvmdb
{
  ...
  public:
    ...
    // number of iterations done for each locNVM of the dedicated mutex.
    // This allows to throttle the locNVM and prevents the starvation
    // of other processes.
    ...
    static void AddParametersToConfigParser(DatabaseOptions& db_options, ConfigParser& parser)
    {
      ...
      parser.AddParameter(new kdb::StringParameter(
        "log.target", "NVMdb", &db_options.log_target, false,
      ...
      parser.AddParameter(new kdb::StringParameter(
        ...
        "The mode with which the write buffer handles incoming traffic, can be 'direct' or 'adaptive'. With the 'direct' mode, once the Write Buffer is full other incoming Write and Delete operations will block until the buffer is persisted to secondary storage. The direct mode should be used when the clients are not subjects to timeouts. When choosing the 'adaptive' mode, incoming orders will be made slower, down to the speed of the writes on the secondary storage, so that they are almost just as fast as when using the direct mode, but are never blocNVM. The adaptive mode is expected to introduce a small performance decrease, but required for cases where clients timeouts must be avoided, for example when the database is used over a network."));
      parser.AddParameter(new kdb::UnsignedInt64Parameter(
